# almost from diffusres pi2pix pipeline
...
class StableDiffusionInstructPix2PixImg2ImgPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        assert (
            image.shape == init_image.shape
        ), "image and init_image must have the same shape"
    ...
    # Copied from diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion.StableDiffusionPipelineImg2Img.prepare_latents
    def prepare_latents(
        self,
        init_image,
        timestep,
        ...
        if isinstance(generator, list):
            init_latents = [
                self.vae.encode(init_image[i : i + 1]).latent_dist.sample(generator[i])
